...
if [ ${USE_CUSTOMER_PROVIDED_CERTIFICATE} == "True" ]; then
  retry helm upgrade -i prophecy --namespace prophecy prophecy/prophecy-marketplace --version 2.1.3 --set customer.controlplane=prophecy --set customer.dataplane=prophecy --set global.prophecy.rootUrl=`echo ${ROOT_URL}` --set platform.ingress-nginx.controller.service.loadBalancerIP=`echo ${LOADBALANCER_IP}` --set platform.ingress-nginx.controller.service.annotations."service\.beta\.kubernetes\.io/azure-load-balancer-health-probe-request-path"=/healthz --set prophecy.userCount=`echo ${INITIAL_USER_COUNT}` --set global.customer.name=`echo ${CUSTOMER_NAME}` --set prophecy.wildcardCertName=prophecy-wildcard-tls-secret --set athena.adminPassword=`echo ${ADMIN_PASSWORD}` --set serviceAccount.create=true --set cloud.provider=AZURE  --set version=2.10.0-4 --set athena.tag=billing --set platform.wildcardCert.useExternal=true --set "athena.envs={ INSTALL_PROPHECY=true,HELM_CLOUD_PROVIDER=EXTERNAL,IMAGE_REGISTRY=gcr.io/prophecy-share,DISABLE_CONTROLCENTER=false,IS_MARKETPLACE_DEPLOYMENT=true,DISABLE_METERING=false,DISABLE_AUTO_UPDATES=true,ENABLE_SIGNUP=false, DISABLE_SHARED_VOLUMES=true, CONTROLPLANE_NAMESPACE=prophecy, DATAPLANE_NAMESPACE=prophecy, ELASTICSEARCH_NAMESPACE=prophecy, WILDCARD_CERTIFICATE_NAME=prophecy-wildcard-tls-secret}"
else
  retry helm upgrade -i prophecy --namespace prophecy prophecy/prophecy-marketplace --version 2.1.3 --set global.prophecy.rootUrl=`echo ${ROOT_URL}` --set platform.ingress-nginx.controller.service.annotations."service\.beta\.kubernetes\.io/azure-load-balancer-health-probe-request-path"=/healthz --set prophecy.userCount=`echo ${INITIAL_USER_COUNT}` --set global.customer.name=`echo ${CUSTOMER_NAME}` --set prophecy.wildcardCertName=prophecy-wildcard-tls-secret --set athena.adminPassword=`echo ${ADMIN_PASSWORD}` --set serviceAccount.create=true --set cloud.provider=AZURE  --set version=2.10.0-4 --set athena.tag=billing --set "athena.envs={INSTALL_PROPHECY=true,HELM_CLOUD_PROVIDER=EXTERNAL,IMAGE_REGISTRY=gcr.io/prophecy-share,DISABLE_CONTROLCENTER=false,IS_MARKETPLACE_DEPLOYMENT=true,DISABLE_METERING=false,DISABLE_AUTO_UPDATES=true,ENABLE_SIGNUP=false, DISABLE_SHARED_VOLUMES=true, CONTROLPLANE_NAMESPACE=prophecy, DATAPLANE_NAMESPACE=prophecy, ELASTICSEARCH_NAMESPACE=prophecy}"
fi
