...
retry helm upgrade -i loki grafana/loki-stack -n prophecy --create-namespace --set loki.persistence.enabled=true,loki.persistence.storageClassName=default,loki.persistence.size=200Gi --force
